...
exports.addperson = async (req, res) => {
  ...
  db.query(
    "INSERT INTO add_authority SET ? ",
    { a_name: user_name, a_username: user_username, a_password: hashedp },
    (error, results) => {
      if (error) {
        console.log(error);
      } else {
...
exports.adminlogin = async (req, res) => {
  try {
    ...
    db.query(
      "SELECT * FROM add_authority WHERE a_username = ?",
      [a_username],
      async (error, results) => {
        console.log(results);
        if (
          !results ||
          !(await bcrypt.compare(a_password, results[0].a_password))
        ) {
...
exports.userlogin = async (req, res) => {
  try {
    ...
    db.query(
      "SELECT * FROM add_user WHERE u_companyemail = ?",
      [u_companyemail],
      async (error, results) => {
        console.log(results);
        if (
...
exports.editinventory = async (req, res) => {
  ...
  db.query(
    "INSERT INTO inventorydetails SET ? ",
    {
      p_name: p_name,
      p_price: p_price,
      p_noproducts: p_noproducts,
      p_sidenote: p_sidenote,
      p_totalprice: p_noproducts * p_price,
    },
    (error, results) => {
      if (error) {
        console.log(error);
...
exports.updateinventory = (req, res) => {
  ...
  var query = db.query(selectquery, [pu_name], function (err, result) {
    ...
    if (result.length > 0) {
      var updatequery =
        "UPDATE inventorydetails set p_price =? , p_noproducts =?, p_sidenote =?, p_totalprice =?  WHERE p_name = ?";
      var query = db.query(
        updatequery,
        [
          pu_price,
...
exports.feedbackform = async (req, res) => {
  ...
  db.query(
    "INSERT INTO complaints SET ? ",
    {
      ...
      c_companyemail: c_companyemail,
      c_complaint: c_complaint,
    },
    (error, results) => {
      if (error) {
        console.log(error);
...
exports.companydataform = async (req, res) => {
  ...
  db.query(
    "INSERT INTO companydetails SET ? ",
    {
...
exports.makeaquotationpt1 = async (req, res) => {
  ...
  db.query(
    "INSERT INTO quotationdetails SET ? ",
    { companyabv: companyabvq },
    (error, results) => {
      if (error) {
        console.log(error);
      } else {
...
exports.makeaquotationpt2 = async (req, res) => {
  ...
  var updatequery =
    "UPDATE quotationdetails set products =?  WHERE products = 'empty'";
...
exports.makeaquotationpt3 = async (req, res) => {
  ...
  var updatequery =
    "UPDATE quotationdetails set quotationnumber =?, validity =?, HSN =?, packingcharges =? , GST =? , freight =?, paymentterms =?   WHERE companyaddress = 'empty'";
  var query = db.query(
    updatequery,
    [qnum, valp, hsn, payc, gst, freightc, payt],
    function (err, result) {
      if (err) {
        console.log(err);
      } else {
...
exports.quotationtable = async (req, res) => {
  ...
  var updatequery =
    "UPDATE quotationdetails set  companyaddress = ?  WHERE companyaddress = 'empty'";
...
// exports.makeaqhalf = (req, res) => {
//   console.log(req.body);
//   const cname = req.body.cname;
//   var selectquery = "CALL getprod1(?)";
//   var query = db.query(selectquery, cname, function (err, rows, fields) {
